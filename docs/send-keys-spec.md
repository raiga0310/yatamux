...
yatamux send-keys --pane 2 "ls\r"      # ペイン 2 に "ls<Enter(CR)>" を送信
yatamux send-keys --pane 2 "q"         # ペイン 2 に "q" を送信
```

> **注意（ビルドについて）**
> リリースビルド（`cargo build --release`）は `windows_subsystem = "windows"` が有効になるため、
> `list-panes` / `send-keys` の stdout/stderr がコンソールに表示されません。
> CLI として使う場合は `--features cli` を付けてビルドしてください:
>
> ```powershell
...
| F-8 | `ipc_send_keys_to_unknown_pane_is_ignored` | 存在しない `PaneId(9999)` に `Input` | レスポンスは返らない（サーバー側で無視される） |
