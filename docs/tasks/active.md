...
- **症状**: `yatamux update` が何も出力せず exit 1 で終了する（yatamux 内・外どちらでも）
- **関連**: C-37「エージェント向け環境変数伝搬」がサブタスク完了扱いになっているが、`env | grep -i yatamux` でペイン内に `YATAMUX_SESSION` 等の環境変数が存在しない。環境変数が設定されていないため、`yatamux update` が IPC パイプ名を特定できない可能性がある。
- **調査ポイント**:
  - `crates/terminal/src/pty.rs` で `YATAMUX_SESSION` / `YATAMUX=1` が実際に `CommandBuilder` にセットされているか確認
  - `src/cli.rs` の `update()` が IPC 接続先（パイプ名）をどう決定しているか確認
  - IPC 接続失敗時のエラーハンドリングが `exit 1` を返すだけになっていないか確認

#### サブタスク

- [ ] `pty.rs` に `YATAMUX_SESSION` / `YATAMUX=1` が設定されているか実コードを確認する
- [ ] `yatamux update` の IPC 接続失敗パスにエラーメッセージを出力するよう修正する
- [ ] ペイン内で env vars が正しく伝搬されることを確認・修正する
