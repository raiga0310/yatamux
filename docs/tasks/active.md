...
### C-40: Windows E2E テスト基盤と主要フロー自動化 【優先度: 高】
...
- [ ] `split-pane` → `send-keys` / `wait-pane` / `exec` の主要 CLI フローを自動化する
- [ ] `subscribe-pane` / `interrupt-pane` / `close-pane` / `terminate-pane` の制御系 smoke を追加する
- [ ] `SaveAndQuit` と次回起動時 restore の end-to-end を自動化する
- [ ] `update` は mock release / staged binary を使う安全な smoke を追加し、`B-1` の再現採取に流用できる形へ寄せる
