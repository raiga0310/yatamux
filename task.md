...
### B-7: ペイン分割後にスクロールオフセットが前のペイン幅を維持して画面が見切れる 【優先度: 中】
...
- **根本原因調査が必要**。
  `scroll_offset` がペインサイズ変更時にリセット・クランプされていない可能性。
- **修正方針**: `WM_SIZE` またはペインリサイズ後に `scroll_offset` を
  `scrollback_len` の範囲内にクランプ（超過分を 0 にリセット）する。

#### サブタスク
- [ ] `window.rs` / `layout.rs`: サイズ変更時に `scroll_offset` を有効範囲にクランプ
- [ ] `docs/test-plan-scroll-offset.md` にテストケースを列挙
...
### C-22: レイアウト CLI 管理コマンド（一覧表示・削除・エクスポート） 【優先度: 低】
...
#### サブタスク
- [ ] `src/cli.rs` に `layout` サブコマンドグループと `list` / `delete` / `export` を追加
- [ ] `layout_config.rs` に `delete_layout(name)` / `export_layout(name)` を追加
- [ ] `cargo test && cargo clippy -- -D warnings && cargo fmt --check`

---

### C-23: レイアウト保存時にペインの起動コマンドも保存する 【優先度: 中】

現状の `layout_to_toml()` はペイン構成（分割方向・比率）のみを TOML に書き出すが、
保存時に各ペインで起動していたアプリのコマンドが記録されないため、
レイアウト再現時にウィンドウ分割だけ復元されてアプリは手動起動が必要になる。

- **概要**: `Pane.title`（またはプロセスのコマンドライン）を保存時に `PaneConfig.command` として記録し、
  レイアウト読み込み時に自動送信できるようにする。
- **課題**: ConPTY から現在実行中のプロセスのコマンドラインを取得する手段が限られる。
  選択肢:
  1. `NtQueryInformationProcess` / `wmic` でフォアグラウンドプロセスのコマンドラインを取得
  2. ペイン作成時に送信したコマンド文字列をサーバー側で保持し、`ServerMessage` 経由で参照
  3. OSC エスケープシーケンス（例: `ESC]0;title` の拡張）でシェルから自己申告させる
  オプション 2（サーバー側でコマンド文字列を保持）が最も確実。
- **用途**: `cargo watch`・`nvim`・`git log` 等の開発ツールをレイアウトごとに自動起動できるようにする。

#### サブタスク
- [ ] `Pane` 構造体に `startup_command: Option<String>` を追加し、`ClientMessage::CreatePane` の `command` を保持
- [ ] `ServerMessage::PaneCreated` または専用クエリでコマンドをクライアントへ通知
- [ ] `PaneStore` で各 `PaneId` → コマンド文字列のマップを管理
- [ ] `layout_to_toml()` で保存時にマップから `command` を引いて出力
- [ ] `docs/test-plan-save-layout.md` にテストケース追記
- [ ] `cargo test && cargo clippy -- -D warnings && cargo fmt --check`
