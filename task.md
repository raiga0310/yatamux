...
### C-21: 外観設定（テーマ・フォント・カラーのファイル管理） 【優先度: 中】
...
#### サブタスク
- [ ] `src/config.rs` に `AppearanceConfig` 構造体を追加（フォント・色設定）
- [ ] `window.rs` の色定数を `AppearanceConfig` から読み込む形にリファクタリング
- [ ] フォント選択ロジックを `font_family` 設定に対応
- [ ] デフォルト値は既存の Catppuccin Mocha 配色を維持
